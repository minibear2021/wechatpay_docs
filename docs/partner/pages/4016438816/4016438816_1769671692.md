...
![](https://gtimg.wechatpay.cn/resource/xres/mmpaydoc/static/img/c87dc0f95c4530b2a18d24fb20b62e6a.png)
...
| [修改商品券](https://pay.weixin.qq.com/doc/v3/partner/4016434633.md) | 是 | 服务商可以通过该接口修改商品券信息。注：修改只会对新发的券生效，历史已经发放给用户的券不会改变。<br>前置条件：已创建商品券 |
| [失效商品券](https://pay.weixin.qq.com/doc/v3/partner/4016434631.md) | 是 | 服务商可以通过该接口使某个商品券失效。<br>注意：<br>- 失效商品券会失效商品券所有批次。<br>  <br>- 失效商品券会同步终止该商品券所有投放渠道和活动，不会有新的用户领券事件，但历史已经通过该商品券对应的批次发放的用户券仍然有效。<br>  <br>前置条件：已创建商品券 |
| 商品券批次管理 | [添加商品券批次组](https://pay.weixin.qq.com/doc/v3/partner/4016280622.md) | 否 | 服务商可以通过该接口为已有的商品券添加更多批次组，多个批次组可以实现服务商多样化的投放需求。<br>前置条件：已创建商品券 |
| [修改商品券批次组](https://pay.weixin.qq.com/doc/v3/partner/4016280633.md) | 否 | 服务商可以通过该接口修改商品券批次组的基本信息，包括展示信息、通知配置。<br>如果你需要调整批次的投放预算，请使用【[修改商品券批次组发放预算API](https://pay.weixin.qq.com/doc/v3/partner/4016280642.md)】<br>前置条件：已创建商品券批次 |
| [修改商品券批次组发放预算](https://pay.weixin.qq.com/doc/v3/partner/4016280642.md) | 否 | 服务商可以通过本接口修改商品券批次组的投放预算。<br>注：本接口每次调用只能调整一个维度的投放预算，如果你需要调整多个维度的预算，请多次调用本接口。<br>前置条件：已创建商品券批次 |
| [批次组批量关联门店](https://pay.weixin.qq.com/doc/v3/partner/4016280620.md) | 否 | 服务商可以通过该接口将品牌的门店列表批量与商品券批次组内所有批次关联<br>前置条件：已创建商品券批次组且批次组的 `store_scope` 为 `SPECIFIC` |
| [批次组批量取消关联门店](https://pay.weixin.qq.com/doc/v3/partner/4016280629.md) | 否 | 服务商可以通过该接口取消品牌的门店列表与商品券批次组内所有批次的关联关系<br>前置条件：已创建商品券批次组且批次组的 `store_scope` 为 `SPECIFIC` |
| 用户商品券管理 | [向用户发放商品券批次组](https://pay.weixin.qq.com/doc/v3/partner/4016280664.md) | 否 | 服务商可以通过本接口向用户发放指定商品券批次组，能否发放受限于商品券批次的发放限额：<br>1. 商品券批次总预算<br>   <br>2. 商品券批次每日预算（如果有）<br>   <br>3. 商品券批次每人限领（如果有）<br>   <br>前置条件：已创建商品券批次，商品券批次处于 `SENDING` 状态<br>注意：配置到“[投放计划](https://pay.weixin.qq.com/doc/v3/partner/4016402231.md)”的批次，用户可通过投放计划领券，领券时由微信侧发放商品券，服务商无需调用本接口再次发券。 |
| [确认发放用户商品券](https://pay.weixin.qq.com/doc/v3/partner/4016435562.md) | 是 | 给用户发券后，微信支付会给服务商发送【[商品券回调通知](https://pay.weixin.qq.com/doc/v3/partner/4016435717.md)】，此时服务商应调用本接口确认发放成功。<br>注：商品券有多个发券渠道，可以由微信支付提供的营销渠道（摇一摇有优惠、商家名片等）发券，服务商也可以调用商品券本身的发券API主动发券，不论由何渠道发放，微信支付均会发送该通知给服务商，服务商均需调用本接口确认发放成功。 |
| [核销用户商户券](https://pay.weixin.qq.com/doc/v3/partner/4016435636.md) | 是 | 服务商可以通过本接口核销已经发放给用户的商品券<br>前置条件：已经给用户发券成功，且用户券当前可用 |
| [失效用户商品券组](https://pay.weixin.qq.com/doc/v3/partner/4016280658.md) | 否 | 服务商可以通过本接口将用户的商品券组失效<br>前置条件：已经给用户发券成功 |
| [退券](https://pay.weixin.qq.com/doc/v3/partner/4016435674.md) | 是 | 服务商可以通过本接口将已经核销用户的商品券退回给用户<br>前置条件：已经给用户发券成功，且用户券当前已核销 |
| 商品券回调通知配置 | [设置商品券事件通知地址](https://pay.weixin.qq.com/doc/v3/partner/4016435719.md) | 是 | 服务商可以通过本接口设置商品券相关事件的回调地址，该地址配置为服务商维度配置，该服务商下的所有商品券相关事件均会通知到该地址。 |
| [获取商品券事件通知地址](https://pay.weixin.qq.com/doc/v3/partner/4016435718.md) | 是 | 服务商可以通过本接口查询商品券相关事件的回调地址，该地址配置为服务商维度配置，该服务商下的所有商品券相关事件均会通知到该地址。<br>前置条件：服务商已经配置商品券事件通知地址 |
...
| 商品券批次管理 | [查询商品券批次列表](https://pay.weixin.qq.com/doc/v3/partner/4016434641.md) | 是 | 服务商可以通过该接口分页查询某个商品券的批次列表。<br>前置条件：已创建商品券 |
| [查询商品券指定批次](https://pay.weixin.qq.com/doc/v3/partner/4016434649.md) | 是 | 服务商可以通过该接口查询某个商品券批次的详情。<br>前置条件：已创建商品券批次 |
| [查询批次关联门店列表](https://pay.weixin.qq.com/doc/v3/partner/4016434665.md) | 是 | 服务商可以通过该接口分页查询商品券批次所关联的门店列表<br>前置条件：已创建商品券批次且批次的 `store_scope` 为 `SPECIFIC` |
| [预上传券Code](https://pay.weixin.qq.com/doc/v3/partner/4016434668.md) | 是 | 服务商可以通过该接口为商品券批次预上传券Code<br>前置条件：已创建商品券批次，商品券批次的 `coupon_code_mode` 配置为 `UPLOAD` |
| 用户商品券管理 | [查询用户商品券详情](https://pay.weixin.qq.com/doc/v3/partner/4016435702.md) | 是 | 服务商可以通过本接口查询已经发放给用户的商品券详情<br>前置条件：已经给用户发券成功 |
| [指定券状态查询用户商品券列表](https://pay.weixin.qq.com/doc/v3/partner/4016435703.md) | 是 | 服务商可以通过本接口查询已经发放给用户的特定状态的商品券<br>前置条件：已经给用户发券成功 |